...
{
    public class CustomerAddSendSMSHandler : INotificationHandler<CustomerAddNotification>
    {
        public async Task Handle(CustomerAddNotification notification, CancellationToken cancellationToken)
        {
            Console.WriteLine($"sending SMS for adding customer: {notification.customer.Name} #{notification.customer.Id}");
